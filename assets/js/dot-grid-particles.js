...
(function () {
  ...
  if (!window.globalDotGridMouse) {
    window.globalDotGridMouse = { clientX: -9999, clientY: -9999, active: false };
    ...
    function animate(time) {
      if (isVisible) {
        ...
        for (var i = 0; i < dots.length; i++) {
          ...
          var dMouseOrigin = Math.sqrt(rxOriginScaled * rxOriginScaled + ryOriginScaled * ryOriginScaled);

          var b = Math.sin(timeSec * 0.8); // Breathing cycle
          var angle = Math.atan2(ryOrigin, rxOrigin);
          var shapeFactor = Math.sin(angle * 2.0 + timeSec * 0.4) * 0.5 + Math.cos(angle * 3.0 - timeSec * 0.2) * 0.25;
          ...
          var vSize = smoothstep(1.8, 0.0, Math.abs(dMouseOrigin - rCurrent) * (2.2 / normalizedMouseRadius));
          ...
          if (vSize > 0.001) {
            ...
            var dxDrift = Math.sin(tDrift + pyUnit * 0.5) + Math.sin(tDrift * 0.5 + pyUnit * 2.0);
            var dyDrift = Math.cos(tDrift + pxUnit * 0.5) + Math.cos(tDrift * 0.5 + pxUnit * 2.0);
            ...
            var oscOuter = Math.sin(timeSec * 2.6 + (driftX / SCALE) * 0.6 + (driftY / SCALE) * 0.6);
          ...
          var sBase = U_PARTICLE_BASE_SIZE + Math.sin(timeSec + (dot.originX / SCALE)) * 0.002 * vSize;
          var currentScale = sBase + (vSize * U_PARTICLE_ACTIVE_SIZE);
          var sizePixels = currentScale * SCALE;

          var stretch = vSize * (U_STRETCH_SCALE * (currentMouseRadius / 220.0)) * SCALE;
          
          // Interpolate Y-scale from 1.0 (circle) to 0.6 (oval) based on rim influence vSize
          var currentBlobScaleY = mixColors({ r: 100, g: 0, b: 0 }, { r: 60, g: 0, b: 0 }, vSize).r / 100.0;

          // 3. Smooth rotation wiggling (only active near cursor)
          var finalAngle = 0;
          if (vSize > 0.01) {
            var osc = 0.5 + 0.5 * Math.sin(timeSec * 0.6 + dot.phi);
            var speedScale = mixColors({ r: 55, g: 0, b: 0 }, { r: 135, g: 0, b: 0 }, osc).r / 100.0;
            var jitterScale = mixColors({ r: 70, g: 0, b: 0 }, { r: 145, g: 0, b: 0 }, osc).r / 100.0;
            var thetaJitter = Math.sin(timeSec * 0.1 * speedScale + (dot.originX / SCALE) * 0.35 + (dot.originY / SCALE) * 0.35) * (0.2 * jitterScale);
          ...
          var finalAlpha = mixColors({ r: 18, g: 0, b: 0 }, { r: 90, g: 0, b: 0 }, alphaTarget).r / 100.0;
          ...
          if (dot.alpha > 0.01) {
            ...
            if (vSize > 0.01) {
              ...
              ctx.fillStyle = "rgba(" + color.r + "," + color.g + "," + color.b + "," + (dot.alpha * 0.08) + ")";
              ctx.fill();
            }
